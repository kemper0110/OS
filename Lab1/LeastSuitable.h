...
struct LeastSuitable {
	...
	//Allocator<Storage, LeastSuitable<T, Storage>>* allocator() {
	//	return static_cast<Allocator<Storage, LeastSuitable<T, Storage>>*>(this);
	//}
};


template<typename T>
auto LeastSuitable<T, Bitmap<T>>::allocate(std::size_t n) {
...
template<typename T>
auto LeastSuitable<T, Bitmap<T>>::deallocate(size_t* data_start) {
